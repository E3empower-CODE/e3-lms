...
> **Current status:** The **frontend is built out** against the planned API — foundations and auth through registration, admissions, student creation, the student and instructor portals, coursework (submission + grading), attendance, progress, finance/payments, and certificates/reporting. It targets the documented contracts (`API.md`), lints, builds, and is covered by unit, MSW integration, and Playwright smoke tests; see [`frontend/FRONTEND_PLAN.md`](./frontend/FRONTEND_PLAN.md) for the phase map. The **Django backend has its foundation in place** — a custom email/role `User`, cookie-session auth with CSRF (`/api/v1/auth/login|logout|me`), a health check, the documented response/error envelopes, CORS, and a generated OpenAPI schema — covered by tests and running on SQLite for local dev. The remaining domain (applications, courses, enrollment, coursework, finance, certificates, reporting) is **not implemented yet**, so most live flows still await their endpoints; frontend paths are provisional and env-overridable. Postgres/Redis are optional locally (SQLite is the default). See `PLAN.md` for authoritative status and order.
