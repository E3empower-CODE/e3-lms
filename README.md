...
> **Current status:** The **frontend is built out** against the planned API — foundations and auth through registration, admissions, student creation, the student and instructor portals, coursework (submission + grading), attendance, progress, finance/payments, and certificates/reporting. It targets the documented contracts (`API.md`), lints, builds, and is covered by unit, MSW integration, and Playwright smoke tests; see [`frontend/FRONTEND_PLAN.md`](./frontend/FRONTEND_PLAN.md) for the phase map. The **Django backend is not implemented yet** (only the project + `accounts` scaffold exist), so live data and end-to-end flows await the API; endpoint paths are provisional and env-overridable. PostgreSQL and Redis are not provisioned locally. See `PLAN.md` for authoritative status and order.
